...
test("keeps the approved Site copy exact", () => {
  ...
  const workedResults = {
    ...
    "brand-studio": "The identity system is usable, its proposals are labeled, and the owner's open decisions are visible.",
    "image-prompting-studio": "The prompt set is complete, copyable, consistent across images, and ready for the team's own generation step.",
  ...
  assert.equal(guideSource.guides["brand-studio"].successSignals[3], "Finished assets are checked together and individually.");
  assert.equal(guideSource.guides["image-prompting-studio"].tips[0], "Name the model when you have one; otherwise you get a broadly compatible prompt.");
