# Identity methods and optional board presets

This reference retains the original brandkit's symbolism, concept methods, visual catalogs and board recipes. Load only the sections needed for the current identity deliverable. Strategy and logo methods apply broadly. The sparse deck treatment and its output defaults apply only when that preset is chosen. Other visual directions, dense information systems and non-board deliverables are valid.
...
For the optional sparse overview preset:

- Describe one brand-kit overview image
- Default layout: `3 × 3`
...
Use supplied or confirmed strategy first. When strategy is open, develop an explicitly proposed interpretation from the available evidence.
...
Use this when the nine-panel identity overview suits the requested board:
...
For sparse presentation boards, use very little text. For guidelines and other information-led deliverables, use the text the reader needs.
...
Use the amount of expression, detail and restraint the selected brand direction calls for.
...
The complete source recipe is in `../skills/brand-identity-system/assets/board-prompt-template.md`. Load it only for a board brief; adapt the selected aesthetic rather than forcing its sparse default.

# FINAL OUTPUT STANDARD

For the sparse identity-board preset, the image should resemble:
