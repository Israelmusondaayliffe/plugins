...
**Peer workflows.** Fable Advisor (agent-ops) is a separate user-invoked top-level workflow. The gauntlet and Fable Advisor never activate, nest, or compose each other automatically. Whichever one the user explicitly invokes owns that run. If the user names both for the same run, or invokes the gauntlet while a Fable Advisor run is active on the target, stop, initialize no state, and ask the user to choose one workflow or end the active run. Detect a Fable Advisor run through `.fable-advisor/runs/*/run-state.json`, bound by `manifest_sha256` to the adjacent immutable `run-manifest.json`. States `completed`, `failed`, and `cancelled` are terminal; `active` blocks a competing run. Missing, malformed, or mismatched state is unresolved, so ask the user to reconcile it before starting. Never infer activity solely from an old manifest or mutate that hash-bound manifest to record completion. Neither workflow silently supersedes the other.
