#!/usr/bin/env python3
"""Validate fail-closed Astra Advisor packets and their inspectable evidence."""
...
def validate_runtime_requirements(value: Any, errors: list[str]) -> None:
    ...
    if set(support) != set(RUNTIME_REQUIREMENTS):
        add(errors, "runtime_requirements.required_model_efforts must name the exact GPT-6 Astra model ID")
...
def validate_topology(value: Any, errors: list[str]) -> None:
    ...
    validate_role(
        topology.get("parent"),
        "topology.parent",
        {"role": "parent", "model": "gpt-6-astra", "reasoning_effort": "xhigh", "may_dispatch_tasks": True},
        errors,
    )
    worker_base = {
        "role": "worker",
    ...
    validate_role(topology.get("routine_worker"), "topology.routine_worker", {**worker_base, "model": "gpt-6-astra"}, errors)
    validate_role(topology.get("complex_worker"), "topology.complex_worker", {**worker_base, "model": "gpt-6-astra"}, errors)
    validate_role(
    ...
    if len(normalized_edges) != len(edges) or normalized_edges != EXPECTED_EDGES:
        add(errors, "topology.edges must be the fixed acyclic Astra Advisor edges")
...
def validate_parent_runtime_attestation(value: Any, run_id: Any, errors: list[str], root: Path | None) -> None:
    ...
    for field, expected in {"role": "parent", "model": "gpt-6-astra", "reasoning_effort": "xhigh", "target": "parent_task"}.items():
    ...
    if source == "host_metadata":
        if attestation.get("effective_model") != "gpt-6-astra":
            add(errors, "RunManifest.parent_runtime_attestation.effective_model must be gpt-6-astra when host metadata is available")
...
def validate_task_authorization(
    ...
    expected_model = "gpt-6-astra" if classification == "routine" else "gpt-6-astra"
    ...
    if authorization.get("reasoning_effort") != "xhigh":
        add(errors, f"{label}.reasoning_effort must be 'xhigh'")
...
def validate_command_records(
    ...
    *,
    ...
    for index, item in enumerate(value):
        ...
        if command_evidence.get("record_type") != "AstraAdvisorCommandEvidence":
            add(errors, f"{item_label}.evidence JSON record_type must be AstraAdvisorCommandEvidence")
