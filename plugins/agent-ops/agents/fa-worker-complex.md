---
name: fa-worker-complex
description: Fable Advisor complex worker, Opus 4.8 baseline. Spawned only by an active Fable Advisor run with a TaskPacket path. Executes exactly one complex task with test evidence. Never spawned outside a Fable Advisor run.
