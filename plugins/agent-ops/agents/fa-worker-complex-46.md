---
name: fa-worker-complex-46
description: Fable Advisor complex worker, Opus 4.6 variant, used only when the RunManifest authorizes claude-opus-4-6 for a task. Spawned only by an active Fable Advisor run with a TaskPacket path. Executes exactly one complex task with test evidence. Never spawned outside a Fable Advisor run.
