"""Focused contract tests for Astra Advisor packets and explicit routing."""
...
SOL_ROOT = ROOT / "skills" / "astra-advisor"
sys.path.insert(0, str(SOL_ROOT / "scripts"))

import importlib.util
_spec = importlib.util.spec_from_file_location("astra_packet_validator", SOL_ROOT / "scripts" / "validate_packets.py")
validate_packets = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(validate_packets)
...
class AstraAdvisorProtocolTests(unittest.TestCase):
    def run_copied_bundle_validator(self, *, spec_plugin: str | None = None) -> subprocess.CompletedProcess[str]:
        temporary = tempfile.TemporaryDirectory()
        self.addCleanup(temporary.cleanup)
        copied_root = Path(temporary.name) / "0.2.0+codex.test-cache"
        shutil.copytree(ROOT, copied_root)
        if spec_plugin is not None:
            spec_path = copied_root / "bundle-spec.json"
            spec = json.loads(spec_path.read_text(encoding="utf-8"))
            spec["plugin"] = spec_plugin
            spec_path.write_text(json.dumps(spec, indent=2) + "\n", encoding="utf-8")
        return subprocess.run(
            [sys.executable, str(copied_root / "scripts" / "verify_bundle.py")],
            cwd=copied_root,
            check=False,
            capture_output=True,
            text=True,
    ...
    def test_bundle_validator_accepts_version_named_plugin_root(self) -> None:
        result = self.run_copied_bundle_validator()
        self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
        self.assertTrue(json.loads(result.stdout)["valid"])

    def test_bundle_validator_rejects_manifest_spec_identity_mismatch(self) -> None:
        result = self.run_copied_bundle_validator(spec_plugin="different-plugin")
        self.assertNotEqual(result.returncode, 0, result.stderr + result.stdout)
        payload = json.loads(result.stdout)
        self.assertFalse(payload["valid"])
        self.assertIn("manifest and bundle spec plugin identities differ", payload["errors"])
    ...
    def write_command_evidence(
        ...
        *,
        ...
        write_json(
            path,
            {
                "record_type": "AstraAdvisorCommandEvidence",
                "protocol_version": 1,
                "run_id": run_id,
                "task_id": task_id,
    ...
    def valid_packets(self, root: Path) -> tuple[dict, dict, dict, dict]:
        ...
        parent["effective_model"] = "gpt-6-astra"
    ...
    def test_run_manifest_requires_the_reviewed_fields_and_host_parent_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            parent_data["effective_model"] = "gpt-5.6-sol"
    ...
    def test_task_runtime_and_authorization_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            wrong_model["worker"]["model"] = "gpt-5.6-sol"
            self.assertIn("TaskPacket.worker.model must be 'gpt-6-astra'", validate_packets.validate_packet(wrong_model, Path(directory)))
    ...
    def test_explicit_route_fixture_validates(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                str(ROOT / "skills" / "agent-ops-router" / "scripts" / "validate_route.py"),
                str(ROOT / "skills" / "agent-ops-router" / "assets" / "astra-advisor-route.json"),
            ],
            check=False,
            capture_output=True,
            text=True,
