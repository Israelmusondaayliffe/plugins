...
This workflow is self-contained inside `unslop-harness-repair`. All resource paths below resolve inside this skill. Writing Quality is not required.
...
**REWRITE**: Use the full four-phase pipeline only after this repair run has an approved repair group. Return the edited text and a short What changed section. Keep diagnostic scores internal unless the user asks for them.
...
Remove em dashes from assistant-authored editable prose. Never run the replacer across a raw harness or plugin file. If needed, run `scripts/unslop-engine/emdash_replacer.py` on a scratch prose-only copy, review the diff, and apply only approved edits. Preserve code, commands, quotations, citations, and other protected exact material even when it contains an em dash.
...
Load `references/unslop-engine/critique-frameworks.md`. Red-team with the audience lens. Repair only weaknesses inside the approved finding group and paths. Record any new out-of-group weakness for later approval.
